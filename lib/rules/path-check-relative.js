...
const { isPathRelative, getSegmentsFromFile, getNormalizedFilePath, getNormalizedPath } = require('../utils/paths');
          ...
          context.report({
            ...
            fix: (fixer) => {
              const normalizedFilePath = getNormalizedFilePath(fileName)
                ...
                .slice(0, -1)
                .join('/');
              const normalizedImportPath = `/${getNormalizedPath(pathImport)}`;
              const result = path.relative(normalizedFilePath, normalizedImportPath)
